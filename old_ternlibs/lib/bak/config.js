...
//
//  Version            :   0.4.13
//  Original Author    :   Loren West <open_source@lorenwest.com>
//  Homepage           :   http://lorenwest.github.com/node-config/
//
//  We (tern.im) copied this file here just for convenience to make some changes
//    Watching the files changes in config folder
...
var Config = function() {
  ...
  WatchFolder.createMonitor(CONFIG_DIR, function(monitor) {
  ...
  var deployment = process.env.NODE_ENV || 'development';
    ...
    var updateConfig = function (fileName) {
    ...
    monitor.on("created", function (f, stat) {
      if (f === runtimeJsonFilename) return;

      var ext = Path.extname(f);
      if (ext in {'.js': '', '.json': '', '.coffee': '', '.yaml': ''} ) {
        console.log("config file created: " + f);
        updateConfig(f);
      }
    ...
    monitor.on("removed", function (f, stat) {
      ...
      var ext = Path.extname(f);
      if (ext in {'.js': '', '.json': '', '.coffee': '', '.yaml': ''} ) {
        console.log("config file removed: " + f);
        updateConfig(f);
      }
...
Config.prototype._parseFile = function(fullFilename) {
  ...
  var checkRuntimeJson = function() {
    if (fullFilename == runtimeJsonFilename) {
      ...
      Utils.mkdirp.sync(CONFIG_DIR, 0777);
