...
Redis.RedisClient.prototype.del_keys = (pattern, next) ->
  ...
  throw new TypeError "pattern required." if isEmpty pattern
...
class Databases
  ...
  @getClient: (dbName) -> 
    throw new TypeError "dbName required." if isEmpty dbName
  ...
  @add: (dbName) -> 
    unless Databases.openedDb[dbName]?
      Databases.openedDb[dbName] = Databases.getClient dbName
    Databases.openedDb[dbName]
